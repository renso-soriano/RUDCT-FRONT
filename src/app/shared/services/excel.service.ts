...
export class ExcelService {
  ...
  public exportAsExcelFile(json: any[], excelFileName: string): void {
    ...
    // Establecer el estilo de relleno verde para la primera fila
    const greenfillStyle = {
      fill: { patternType: "solid", fgColor: { rgb: "FF00FF00" } } // RGB para verde
    };
    columnNames.forEach((columnName, columnIndex) => {
        const cellRef = XLSX.utils.encode_cell({ r: 0, c: columnIndex }); // Celda en la primera fila
        worksheet[cellRef].s = greenfillStyle; // Aplicar el estilo de relleno verde
    });

    // Definir el ancho específico para algunas columnas
